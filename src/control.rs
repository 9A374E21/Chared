...
pub fn 下移(
    光标: &mut 光标, 行向量: &[String], 最大行数: usize, 下: bool
) -> io::Result<()> {
    if 行向量.is_empty() {
        return Ok(());
    }
...
pub fn 右移(光标: &mut 光标, 行向量: &[String], 右: bool) {
    let 当前行号 = 光标.行索引 + 光标.行 as usize;
    if 当前行号 < 行向量.len() {
        if 右 {
            let 符宽 = 当前字符宽度(行向量[当前行号].as_str(), 光标.列);
            let 新列 = 光标.列 + 符宽 as u16;
            let 行长 = 句宽!(行向量[当前行号].as_str());
            if 新列 <= (行长 as u16) {
                光标.列 = 新列;
                光标.列历史 = 光标.列;
...
fn 调整光标列(行向量: &[String], 光标: &mut 光标) {
    let 行内容 = 行向量[光标.行索引 + 光标.行 as usize].as_str();
