    ...
    if 当前行号 < 行向量.len() {
        if 右 {
            ...
            if 光标.列 < (行长 as u16) {
                ...
                // 已到最右侧，继续按右键时往下移
                control::下移(光标, &行向量, 最大行数, true, Some(true)).ok();
            }
        } else {
            if 光标.列 > 0 {
                let mut 符宽 = 0usize;
                ...
                // 已到最左侧，继续按左键时往上移
                control::下移(光标, &行向量, 最大行数, false, Some(false)).ok();
            }
        }
    }
}
...
fn 调整光标列(行向量: &[String], 光标: &mut 光标, 归零: Option<bool>) {
    // 确保索引不越界
    let 行索引 = std::cmp::min(光标.行索引 + 光标.行 as usize, 行向量.len() - 1);
    let 行内容 = 行向量[行索引].as_str();
