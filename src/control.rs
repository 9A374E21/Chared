...
pub fn 下移(
    ...
    if 新行 != 光标.行 {
        光标.行 = 新行;
        光标.列 = std::cmp::min(
            调整列到字符边界(行向量[光标.行索引], 光标.列历史),
            句宽!(行向量[光标.行索引]) as u16,
        );
    }
    ...
    if 下 && 原行 == (最大行数 as u16 - 1) && (光标.行索引 + 最大行数) < 行向量.len()
    {
        ...
        // 重新定位列（使用列历史）
        let 当前行 = 行向量[光标.行索引 + 光标.行 as usize];
        光标.列 = std::cmp::min(调整列到字符边界(当前行, 光标.列历史), 句宽!(当前行) as u16);
    }
...
pub fn 下翻(
    ...
    光标.列 = 新列;
    光标.行索引 += 光标.行 as usize;

    Ok(())
}

pub fn 右移(光标: &mut 光标, 行向量: &[&str], 右: bool) {
    let 当前行索引 = 光标.行索引 + 光标.行 as usize;
    if 当前行索引 < 行向量.len() {
        let 行 = 行向量[当前行索引];
        if 右 {
            // 右移
